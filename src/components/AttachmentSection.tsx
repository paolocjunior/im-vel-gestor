...
import { Paperclip, Upload, X, Download, ChevronDown, ChevronRight } from "lucide-react";
          ...
          <div className="mt-2 space-y-1">
            ...
            {attachments.map(doc => (
              <div key={doc.id} className="flex items-center gap-1 text-xs bg-muted rounded px-2 py-0.5">
                ...
                <button onClick={() => handleDownload(doc)} className="text-primary hover:text-primary/80">
                  <Download className="w-3 h-3" />
                </button>
                {!readOnly && (
                  <button onClick={() => handleDelete(doc)} className="text-destructive hover:text-destructive/80">
                    <X className="w-3 h-3" />
                  </button>
                )}
              </div>
